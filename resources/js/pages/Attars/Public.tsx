                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-12 mb-16">
                            {attars.data.length > 0 ? (
                                attars.data.map((attar) => {
                                    let image = attar.image_path;
                                    const detail = attar.attar_detail;
                                    if (!image && detail?.images && detail.images.length > 0) {
                                        image = detail.images[0];
                                    }

                                    return (
                                        <Link key={attar.id} href={route('attars.show', attar.id)} className="group block bg-[#080808] p-3 rounded-[2.5rem] border border-white/5 hover:border-amber-500/20 transition-all duration-500 shadow-2xl">
                                            ...
                                            <div className="mt-5 px-3 pb-2">
                                                ...
                                                <div className="flex items-center justify-between border-t border-white/5 pt-4 mt-2">
                                                    <span className="text-white font-black text-2xl tracking-tighter">
                                                        ₹{Number(attar.price).toLocaleString()}
                                                    </span>

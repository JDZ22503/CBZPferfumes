...
export default function Index({ details, filters }: Props) {
    ...
    const handleToggleStatus = (id: number) => {
        router.post(route('gift-set-details.toggle-status', id), {}, {
            preserveScroll: true,
        });
    };

    const handleToggleFeatured = (id: number) => {
        router.post(route('gift-set-details.toggle-featured', id), {}, {
            preserveScroll: true,
        });
    };
    ...
    const handleDelete = (id: number) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.delete(route('gift-set-details.destroy', id), {
                    onSuccess: () => {
                        Swal.fire(
                            'Deleted!',
                            'Gift set details have been deleted.',
                            'success',
                        );
                    },
                });
            ...
            <div className="py-6">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="mb-6 flex items-center justify-between">
                        ...
                        <Link
                            href={route('gift-set-details.create')}
                                    ...
                                    <tr key={detail.id}>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm font-medium text-gray-900 dark:text-white">
                                                {detail.product_set.name}
                                            </div>
                                            <div className="text-xs text-gray-500 dark:text-gray-400">
                                                {detail.product_set.sku}
                                            </div>
                                        </td>

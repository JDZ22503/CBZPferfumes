...
import { type ProductSet, type PaginatedData } from '@/types';
...
export default function PublicProductSets({
    productSets,
}: {
    productSets: PaginatedData<ProductSet>;
}) {
    return (
        <>
            <Head title="Gift Sets - CBZ Perfumes" />
                ...
                <div className="pt-32 pb-12 bg-gradient-to-b from-zinc-900 to-black">
                    <div className="container mx-auto px-6 text-center">
                        ...
                        <p className="text-gray-400 max-w-2xl mx-auto">
                            Exquisite gift sets for your loved ones.
                        </p>
                    </div>
                </div>
                ...
                <section className="py-12 relative overflow-hidden min-h-[50vh]">
                    <div className="container mx-auto px-6">
                        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-8 mb-16">
                            {productSets.data.length > 0 ? (
                                productSets.data.map((set) => (
                                    <Link key={set.id} href={route('product-sets.show', set.id)} className="group relative bg-black/40 rounded-3xl overflow-hidden border border-white/5 hover:border-amber-500/30 transition-all duration-500">
                                        <div className="aspect-[4/5] bg-zinc-900 relative overflow-hidden">
                                            {set.image_path ? (
                                                <img
                                                     src={`${import.meta.env.VITE_API_BASE_URL}${set.image_path}`}
                                                    alt={set.name}
                                        ...
                                        <div className="p-6">
                                            <div className="flex justify-between items-start mb-2">
                                                <h3 className="font-bold text-xl group-hover:text-amber-500 transition-colors truncate pr-2">{set.name}</h3>
                                            </div>
                                            <p className="text-sm text-gray-500 mb-4 uppercase tracking-[0.2em] font-medium">Premium Collection</p>
                                            <div className="flex items-center justify-between pt-4 border-t border-white/5">
                                                <span className="font-bold text-white text-xl">₹{Number(set.price).toLocaleString()}</span>

...
export default function FrontendFooter() {
    const { contactDetails, app_settings } = usePage<any>().props;
        ...
        <footer className="bg-black border-t border-white/10 py-12">
            <div className="container mx-auto px-6">
                <div className="grid md:grid-cols-4 gap-12 mb-12">
                    <div className="space-y-6">
                        ...
                        <div className="space-y-3 mt-4">
                            <div className="text-gray-400 text-xs flex items-start gap-3">
                                <div className="w-5 h-5 flex items-center justify-center bg-amber-500/10 rounded text-amber-500 shrink-0">
                                    <MapPin className="w-3 h-3" />
                                </div>
                                <span>{contactDetails?.address || '123 Luxury Avenue, Fragrance District, Gujarat'}</span>
                            </div>
                            <div className="text-gray-400 text-xs flex items-center gap-3">
                                <div className="w-5 h-5 flex items-center justify-center bg-amber-500/10 rounded text-amber-500 shrink-0">
                                    <Phone className="w-3 h-3" />
                                </div>
                                <span>{contactDetails?.phone || '+91 123 456 7890'}</span>
                            </div>
                            <div className="text-gray-400 text-xs flex items-center gap-3">
                                <div className="w-5 h-5 flex items-center justify-center bg-amber-500/10 rounded text-amber-500 shrink-0">
                                    <Mail className="w-3 h-3" />
                                </div>
                                <span>{contactDetails?.email || 'info@cbzperfumes.com'}</span>
                            </div>
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <ul className="space-y-2 text-sm text-gray-500">
                            ...
                            <li><Link href={route('terms')} className="hover:text-white transition-colors">Terms & Conditions</Link></li>
                            <li><Link href={route('returns')} className="hover:text-white transition-colors">Shipping & Returns</Link></li>
                        </ul>
                    </div>
                    {(contactDetails?.instagram_link || contactDetails?.whatsapp_link || contactDetails?.facebook_link) && (
                    <div>
                        ...
                        <div className="flex gap-4">
                            {contactDetails?.instagram_link && (
                                <a 
                                    ...
                                    className="w-10 h-10 bg-white/5 rounded-full flex items-center justify-center text-gray-400 hover:bg-amber-500 hover:text-black transition-all"
                                    title="Instagram"
                                >
                            ...
                            {contactDetails?.whatsapp_link && (
                                <a 
                                    href={contactDetails.whatsapp_link} 
                                    target="_blank" 
                                    rel="noopener noreferrer"
                                    className="w-10 h-10 bg-white/5 rounded-full flex items-center justify-center text-gray-400 hover:bg-green-500 hover:text-white transition-all"
                                    title="WhatsApp"
                                >
                            ...
                            {contactDetails?.facebook_link && (
                                <a 
                                    href={contactDetails.facebook_link} 
                                    target="_blank" 
                                    rel="noopener noreferrer"
                                    className="w-10 h-10 bg-white/5 rounded-full flex items-center justify-center text-gray-400 hover:bg-blue-600 hover:text-white transition-all"
                                    title="Facebook"
                                >
                ...
                <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-gray-600">
                    <p>&copy; {new Date().getFullYear()} {app_settings?.company_name || 'CBZ Perfumes'}. All rights reserved.</p>

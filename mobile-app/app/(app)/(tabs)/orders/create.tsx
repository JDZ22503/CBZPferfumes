...
import { View, ScrollView, StyleSheet, Alert, ActivityIndicator, TouchableOpacity, Text, Modal, FlatList, TextInput, KeyboardAvoidingView, Platform } from 'react-native';
    ...
    const onSubmit = async (data: any) => {
        try {
            ...
            const payload = {
                ...
                paid_amount: data.payment_status === 'partial' ? parseFloat(data.paid_amount) || 0 : 0,
            };
            ...
            <ScrollView contentContainerStyle={styles.container}>
                ...
                <View style={[styles.row, { zIndex: 1 }]}>
                    <View style={{ flex: 1, marginRight: 10 }}>
                        ...
                        {showDatePicker && (
                            <DateTimePicker
                                value={watch('order_date') ? new Date(watch('order_date')) : new Date()}
                                mode="date"
                                display={Platform.OS === 'ios' ? 'spinner' : 'default'}
                                onChange={(event, date) => {
                                    setShowDatePicker(false);
                                    if (date) {
                                        setValue('order_date', date.toISOString().split('T')[0]);
                                    }
                                }}
                            />
                        )}
                        ...
                        <View style={{ marginTop: 10 }}>
                            ...
                            <View style={styles.paidAmountRow}>
                                ...
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <TextInput
                                            ...
                                            onFocus={() => {
                                                if (field.value === '0' || field.value === '0.00') {
                                                    field.onChange('');
                                                }
                                            }}
                                            onBlur={() => {
                                ...
                                <Text style={styles.paidAmountSuffix}>
                                    of ₹{grandTotal.toFixed(2)}
                                </Text>
                            </View>
                            {parseFloat(watch('paid_amount') || '0') > 0 && (
                                <Text style={styles.paidAmountRemaining}>
                                    Remaining: ₹{Math.max(0, grandTotal - parseFloat(watch('paid_amount') || '0')).toFixed(2)}
                                </Text>
                            )}
                        </View>
                    )}

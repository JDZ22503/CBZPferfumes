                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-12 mb-16">
                            {productSets.data.length > 0 ? (
                                productSets.data.map((set) => {
                                    let image = set.image_path;
                                    const detail = set.product_set_detail;
                                    if (!image && detail?.images && detail.images.length > 0) {
                                        image = detail.images[0];
                                    }

                                    return (
                                        <Link key={set.id} href={route('product-sets.show', set.id)} className="group block bg-[#080808] p-3 rounded-[2.5rem] border border-white/5 hover:border-amber-500/20 transition-all duration-500 shadow-2xl">
